...
let paragraphDisplay=document.querySelector(".paragraph-list")
form.addEventListener('submit',(e)=>{
  e.preventDefault();
    let totalPara=form.querySelector(`input[type="number"]`);
    let groupPara=text.slice(0,totalPara.value);
    paragraphDisplay.innerHTML="";
    if (totalPara.value > 0 && totalPara.value<text.length){
      groupPara.forEach(singlePara=>{
        paragraphDisplay.innerHTML+=`<p>${singlePara}</p>`
      })
    }
    else{
      paragraphDisplay.innerHTML="<h1>Input a number between 1 to 8</h1>"
    }
